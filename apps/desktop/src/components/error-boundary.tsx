...
// that just shrank (session switch / teardown / context compression), throwing
// a render-race error that latches the WHOLE app on the root "Reload window"
// screen. These throws clear themselves on the next render against fresh state,
// so the root boundary recovers itself once the storm settles instead of
// stranding the user. The error was renamed from `tapClientLookup` to
// `useClientLookup` in @assistant-ui — match both.
const RECOVERABLE_ERROR_PATTERNS = [
  /(?:tapClientLookup|useClientLookup)[:\s].*out of bounds/i,
