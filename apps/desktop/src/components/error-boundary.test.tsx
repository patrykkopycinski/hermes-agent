...
// (session switch / context compression render race), reproduced verbatim so
// the recoverable-pattern match is exercised against the actual error text.
// The error was renamed from `tapClientLookup` to `useClientLookup` in
// @assistant-ui — test both.
const TAP_ERROR = 'tapClientLookup: Index 23 out of bounds (length: 18)'
const USE_CLIENT_LOOKUP_ERROR = 'useClientLookup: Index 89 out of bounds (length: 12)'
...
describe('ErrorBoundary root auto-recovery', () => {
  ...
  it('recovers the root boundary from a transient stale-index render race (tapClientLookup)', async () => {
  ...
  it('recovers the root boundary from a transient stale-index render race (useClientLookup)', async () => {
    vi.spyOn(console, 'error').mockImplementation(() => {})
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {})
    const box: { error: Error | null } = { error: new Error(USE_CLIENT_LOOKUP_ERROR) }
    const Bomb = makeBomb(box)
